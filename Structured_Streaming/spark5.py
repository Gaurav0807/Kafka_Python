...
if __name__=="__main__":
    ...
    windowAggDF = redefinedOrdersDf.groupBy(window(F.col("order_date"),"15 minute")).agg(F.sum("amount").alias("totalInvoice"))

    resultDF  = windowAggDF.select("window.start","window.end","totalInvoice")

    orderQuery = resultDF.writeStream.format("console").outputMode("update").option("chckpointLocation","chk-location1")\
